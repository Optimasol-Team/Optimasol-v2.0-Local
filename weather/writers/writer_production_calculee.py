"""
Le but de ce script est est d'héberger une fonctio qui prend en argument un dataframe panda (celui généré par le 
module PVlib) et le charge dans la table SQLite production de la BDD optimaosl.db.
Le code est pratiquement le même que weiter_external car les deux insèrent la production. 
...
def write_production_calculee_to_db(df: pd.DataFrame, fournisseur : str, methode_calcul : str) :
    ...
    delete_rows_from_date_external(path_to_db, fournisseur, now)    
    ...
    for index, row in df.iterrows() :
        ...
        try :
            ...
            data_tuple = data_tuple + ("Internal", fournisseur, methode_calcul)
