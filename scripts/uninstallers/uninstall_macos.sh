...
if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "Erreur: ce desinstalleur est prevu pour macOS." >&2
  exit 1
fi
...
echo "Bienvenue dans le desinstalleur Optimasol (macOS)"
if [[ -z "${APP_DIR:-}" ]]; then
...
if [[ "$MOSQ_INSTALLED_BY_OPTIMASOL" -eq 1 ]]; then
  if command -v brew >/dev/null 2>&1; then
    ...
    echo "Attention: Homebrew introuvable; mosquitto non desinstalle automatiquement."
  fi
